...
> **文档定位：** 本文档是《Rust GUI 框架总体设计》（D0）的子系统详细设计，定义组件模型的完整 trait 体系、声明式视图数据类型、组件生命周期、第三方组件协议和 `ui!` 宏设计。
...
| ui! 宏展开 | 编译时展开 ui! {} 并验证类型 | 展开为 WidgetView\<M\> |
