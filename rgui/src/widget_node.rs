...
use rgui_state::diff::{diff, WidgetIdMap};
use rgui_state::Patch;
use rgui_core::view::WidgetView;
use rgui_core::traits::AppMessage;
use rgui_core::context::{ViewContext, UpdateContext};
use std::fmt;
...
pub struct WidgetNode<M: AppMessage> {
    ...
    pub bounds: Rect,
    /// 事件处理器：接收消息名 → 更新状态。
    pub on_event: Box<dyn FnMut(&str, &mut UpdateContext) + Send>,
    /// 视图生成器：根据当前状态生成 WidgetView。
    pub view_fn: Box<dyn Fn(&ViewContext) -> WidgetView<M> + Send>,
    /// 上一次生成的视图（用于 diff）。
...
impl<M: AppMessage> WidgetNode<M> {
    ...
    pub fn new(
        ...
        Self { id, bounds, on_event: Box::new(on_event), view_fn: Box::new(view_fn), prev_view: None }
    }
    ...
    pub fn diff_and_update(&mut self, ctx: &ViewContext, id_map: &mut WidgetIdMap) -> Vec<Patch<M>> {
