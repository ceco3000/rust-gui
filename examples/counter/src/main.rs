...
use rgui::WidgetSpec;
use rgui::{
    WidgetId, Rect, Size, ViewContext,
    LabelState, Label,
};
use rgui::diff::{diff, WidgetIdMap};
use std::sync::{Arc, Mutex};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut app = App::new(AppConfig::new().title("rgui 组件库演示").window_size(400.0, 300.0));
    ...
    let label_state = Arc::new(Mutex::new(LabelState { text: "计数: 0".into() }));
    ...
    app.register_interaction(WidgetId::from_u64(1), Rect::new(50.0, 80.0, 100.0, 40.0), "+1",
        move |_| { let mut c = c1.lock().unwrap(); *c += 1; ls1.lock().unwrap().text = format!("计数: {}", *c); println!("  → {}", *c); });

    // [重置] 交互
    let ls2 = Arc::clone(&label_state);
    let c2 = Arc::clone(&count);
    app.register_interaction(WidgetId::from_u64(2), Rect::new(170.0, 80.0, 100.0, 40.0), "reset",
        move |_| { *c2.lock().unwrap() = 0; ls2.lock().unwrap().text = "计数: 0".into(); println!("  → 重置"); });
